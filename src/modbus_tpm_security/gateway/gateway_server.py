import socket
import threading
from src.modbus_tpm_security.gateway import gateway_common
from src.modbus_tpm_security import utils
from time import sleep
from src.modbus_tpm_security.key_exchange import key_exchange_routine
from src.modbus_tpm_security.perf_measure import latency_measure
from src.modbus_tpm_security.parse_args import parse_args_main
from src.modbus_tpm_security.sym_cipher import SymCipher, CipherTypes
...
def forward_source_dest(communicator_source : NetComm, communicator_dest : NetComm, cipher : SymCipher, latency_meter : latency_measure.LatencyMeter | None = None):
    ...
    while not exit_flag and not reset_flag:
        try:
            enc_data = communicator_source.receive()
            if not enc_data:
...
def forward_dest_source(communicator_source : NetComm, communicator_dest : NetComm, cipher : SymCipher, latency_meter : latency_measure.LatencyMeter | None = None):
    ...
    while not exit_flag and not reset_flag:
        try:
            data = communicator_dest.receive()
            if not data:
                raise ConnectionError
        except TimeoutError:
            if exit_flag or reset_flag:
            ...
            communicator_source.send(cipher.encrypt_and_digest(gateway_common.SOCKET_RESET_MESSAGE, is_reset_msg=True))
            break
        ...
        try:
            communicator_source.send(enc_data)
        except BrokenPipeError:
            reset_flag = True
...
def handle_transfer(args, source_socket : socket.socket, dest_socket : socket.socket, sym_key : bytes):
    ...
    communicator_dest = NetComm(dest_socket, blind_operation=True)
